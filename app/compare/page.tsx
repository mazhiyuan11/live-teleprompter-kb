...
export const metadata: Metadata = createSEOMetadata({
  title: '提词器对比指南 | 硬件、App、中端、高预算工程级怎么选',
  description: '提词器对比指南：硬件vs手机App、中端vs入门、中端vs高预算工程级、便携vs落地式。帮你找到最适合自己场景的提词方案，不花冤枉钱。',
...
const compares = [
  ...
  { title: '中端提词器 vs 高预算工程级', desc: '高预算工程设备和直播间设备差在哪？哪些功能你不需要。', href: '/brand/live-teleprompter', icon: '🏢' },
  ...
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        description: '硬件vs App、中端vs入门、中端vs高预算工程级、便携vs落地式',
      ...
      <div className="container-article py-12 md:py-16">
        ...
        <div className="mb-10"><span className="tag mb-3">对比指南</span>
          ...
          <p className="text-xl text-brand-700 font-medium">硬件、App、中端、高预算工程级——帮你找到最适合的方案</p>
        </div>
        <div className="bg-gradient-to-r from-brand-50 to-blue-50 border border-brand-200 rounded-xl p-6 mb-10">
          
          <p className="text-ink-800 leading-relaxed">提词器选择的核心是两件事——用什么形态（软件App还是硬件）和什么价位（入门、中端还是高预算工程级）。手机App适合偶尔使用、零成本入门。中端硬件提词器适合每天固定使用的中小团队，在稳定性和成本间取得平衡。高预算工程级设备适合电视台和大型演播室，个人主播不需要。</p>
        </div>
        {/* 对比表 */}
        <div className="bg-white border border-ink-100 rounded-xl overflow-hidden mb-10">
          <table className="w-full text-sm">
            <thead><tr className="bg-ink-50"><th className="text-left p-3">对比维度</th><th className="p-3">App/入门</th><th className="p-3 text-brand-700 font-medium">中端（直播猿）</th><th className="p-3">高预算工程级</th></tr></thead>
            <tbody>
              {[ ['价格','0-300元','800-2000元','万元以上'], ['使用场景','偶尔使用','每天固定使用','大型项目空间'], ['眼神效果','视线偏移','看镜头=看稿','极致光学'], ['部署','零门槛','十分钟装好','需工程师调试'], ['适合','个人偶尔用','中小团队/直播间','专业制作团队'] ].map((r,i) => (
